...
class LLMController:
    ...
    def single_completion(self,
        ...
        typed_messages: list[ChatCompletionSystemMessageParam |
                             ChatCompletionUserMessageParam |
                             ChatCompletionAssistantMessageParam] = []
        ...
        system_message = ChatCompletionSystemMessageParam(role="system", content=system_input)
...
def generate_answer(*, session: Session, owner_id: int, conv_id: int) -> CnvMessage:
    ...
    generated_answer = llm.single_completion(system_input=SystemPrompts.assistant, messages_list=conversation.messages)
...
def generate_summary(*, session: Session, conv_id: int) -> str | None:
    ...
    llm = LLMController()
    return llm.single_completion(system_input=SystemPrompts.summary_generator,
